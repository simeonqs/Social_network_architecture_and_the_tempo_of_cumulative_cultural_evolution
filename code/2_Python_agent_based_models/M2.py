...
def compare_notes(G, dyad):
    ...
    j_inventory = G.nodes[dyad[1]]["data"].inventory[:,2]
    #print("inventory2 {}".format(j_inventory))
    combined_inventory = np.clip(i_inventory+j_inventory,a_min=0,a_max=1)
    #print("combined inventory {}".format(combined_inventory))

    G.nodes[dyad[0]]["data"].inventory[:,2] = combined_inventory
    G.nodes[dyad[1]]["data"].inventory[:,2] = combined_inventory
...
def create_csv():
    ...
    with open("m2/data_"+str(graph_type)+".csv","w") as f:
...
def create_csv_proportions():
    #writes header for main data
    with open("m2/proportions_"+str(graph_type)+".csv","w") as f:
...
def write_csv(sim_num,timestep,epoch,graph_condition,pop_size,agent_i,agent_j,discovery,innov_level,a_track,b_track):
    #writes row of data
    with open("m2/data_"+str(graph_type)+".csv","a") as f:
            ...
            while not crossover_diffused: #force sims to continue running
                ...
                for focal in focal_list:
                    ...
                    if discovery:
                        ...
                        if discovery in [7,14] and crossover_found==False:
                            print("Crossover found at timestep {}, epoch {}.".format(timestep, epoch))
                            crossover_found = True
                    timestep +=1
                crossover_diffused = count_pockets(G,sim_num,epoch,graph_condition,N,crossover_diffused)
                epoch+=1
            else:
